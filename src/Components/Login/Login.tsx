...
import { EMAIL_REQ, PWD_REQ } from "../../constants";
...
const Login: React.FunctionComponent = () => {
  ...
  //   });
  const validationSchema = Yup.object().shape({
    email: Yup.string().required(EMAIL_REQ),
    password: Yup.string().required(PWD_REQ),
  });
  const {
    handleSubmit,
    control,
        ...
        <form>
          <div className="form-group">
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <Input size="large" placeholder="Enter your email" {...field} />
              )}
            />
          ...
          <Button color="primary" className="login-btn" onClick={handleSubmit(onSubmit)}>
            Sign In
          </Button>
        </form>
      </Card>
